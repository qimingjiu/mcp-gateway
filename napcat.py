    ...
    try:
        ...
        def _check():
            ...
            if all_chats and all_chats.data and len(all_chats.data) >= threshold:
                ...
                # ⚡ 关键修复：总结调用不能跟主对话共用同一个 DeepSeek Key！
                # 原因：DeepSeek 的 prefix cache 是按 API Key 限额的缓存池，不是按会话隔离的。
                # 这里每次总结都会一次性发送一个几千~上万 token 的全新大 prompt，
                # 如果用主对话的 CHAT_API_KEY，这次性大请求会挤占（LRU 淘汰）主对话已经缓存好的前缀，
                # 导致触发总结的那一轮，主对话缓存突然从几千 token 跌到几百 token。
                #
                # 解决：优先用独立的 SUMMARY_API_KEY（同账号申请第二个 key 即可，模型可以一样），
                # 让总结请求走完全独立的缓存池，不干扰主对话缓存。
                # 没配置 SUMMARY_API_KEY 时，退回用 main_chat（行为不变，兼容旧配置）。
                summary_api_key = os.environ.get("SUMMARY_API_KEY", "").strip()
                if summary_api_key:
                    from openai import OpenAI
                    summary_base_url = os.environ.get("SUMMARY_BASE_URL") or os.environ.get("CHAT_BASE_URL", "https://api.deepseek.com/v1")
                    summary_model_name = os.environ.get("SUMMARY_MODEL_NAME") or os.environ.get("CHAT_MODEL_NAME", "deepseek-chat")
                    client = OpenAI(api_key=summary_api_key, base_url=summary_base_url)
                    client.custom_model_name = summary_model_name
                    _naplog("📦 总结使用独立 SUMMARY_API_KEY，不占用主对话缓存池")
                else:
                    # 用户要求：总结类一律用聊天模型（main_chat），不用便宜/默认模型
                    client = dep._get_llm_client("main_chat")
                    _naplog("⚠️ 未配置 SUMMARY_API_KEY，总结将与主对话共用缓存池（建议配置独立 key）")
                if client:
                    try:
