...
class HostFixMiddleware:
    ...
    async def _handle_openai_proxy(self, scope, receive, send):
        ...
        if path == "/v1/models" and method == "GET":
            ...
            raw_models = os.environ.get("CHAT_MODELS", "").strip()
            if raw_models:
                model_ids = [m.strip() for m in raw_models.split(",") if m.strip()]
            else:
                model_ids = [default_model]
            models = [
                {"id": model_id, "object": "model", "created": int(time.time()), "owned_by": "mcp-gateway"}
    ...
    async def _handle_chat(self, scope, receive, send):
        ...
        # DeepSeek 官方 API：传稳定 user_id，帮助 KVCache 隔离；不强制修改 thinking 模式。
        # 不要把邮箱、手机号、真实姓名放进 user_id。
        if "deepseek" in upstream_url.lower() or os.environ.get("DEEPSEEK_COMPAT", "").strip().lower() in ("1", "true", "yes", "on"):
            ds_user_id = (
            ...
            try:
                with requests.post(upstream_url, headers=fwd_headers, json=req_data, stream=True, timeout=300) as resp:
                    if resp.status_code != 200:
                        ...
                        q.put(None)
                        return
                    stream_chunk_size = int(os.environ.get("STREAM_CHUNK_SIZE", "512"))
                    for line in resp.iter_lines(chunk_size=stream_chunk_size, decode_unicode=True):
                        if line:
                            q.put(line)
                q.put(None)
            except Exception as e:
        ...
        if sb and user_msg and (collected_content or tool_calls_dict):
            ...
            def _cleanup_save_task(t):
                _pending_save_tasks.discard(t)
                try:
                    exc = t.exception()
                    if exc:
                        _log(f"⚠️ 后台存库 task 异常: {exc}")
                except asyncio.CancelledError:
                    pass
            task.add_done_callback(_cleanup_save_task)
    ...
    async def _save_conversation(self, sb, user_msg, ai_msg, reasoning, tool_calls):
        ...
        save_reasoning = os.environ.get("SAVE_REASONING", "false").strip().lower() in ("1", "true", "yes", "on")
