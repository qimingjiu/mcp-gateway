...
def _pure_reasoning_chunk_info(parsed_chunk):
    if not isinstance(parsed_chunk, dict):
        return None
    if set(parsed_chunk) - {"id", "object", "created", "model", "choices"}:
        return None
    choices = parsed_chunk.get("choices")
    if not isinstance(choices, list) or len(choices) != 1:
        return None
    choice = choices[0]
    if not isinstance(choice, dict) or set(choice) - {"index", "delta", "finish_reason"}:
        return None
    delta = choice.get("delta")
    if not isinstance(delta, dict) or set(delta) - {"reasoning_content", "content", "tool_calls"}:
        return None
    ...
    async def _handle_chat(self, scope, receive, send):
        ...
        while True:
            ...
            if pure_reasoning is not None:
                if reasoning_raw_chunk_count == 1:
