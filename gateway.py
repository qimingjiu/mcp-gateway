...
class HostFixMiddleware:
    ...
    async def _handle_chat(self, scope, receive, send):
        ...
        while True:
            ...
            if chunk == "data: [DONE]":
                if not received_done:
                    received_done = True
                    sse_line_count += 1
                    if not await _send_asgi({"type": "http.response.body", "body": b"data: [DONE]\n\n", "more_body": True}):
                        break
                break
        ...
        # 结束响应：无论 client_disconnected 状态如何，都必须发 more_body:False。
        # 原因：如果中途某次 _send_asgi 抛异常（如瞬时网络抖动）设了 client_disconnected=True，
        # 跳过这一句会导致 ASGI 流没有正常关闭，uvicorn 只能强发 RST_STREAM，
        # rikkahub 就看到 PROTOCOL_ERROR。try/except 兜底，失败了也不影响后续逻辑。
        try:
