...
class HostFixMiddleware:
    ...
    async def _inject_context(self, req_data, sb, current_query):
        ...
        - 动态内容放后面：时间 / 沉默时长 / Pinecone 本轮检索 / 最近历史
        - 通过环境变量控制注入量，减少 DeepSeek prompt cache 被动态前缀打碎
        """
        ...
        # ===== 缓存友好开关 =====
        # CACHE_FRIENDLY_MODE=true 时：时间默认只注入日期，不注入分钟级时间。
        cache_friendly = os.environ.get("CACHE_FRIENDLY_MODE", "true").strip().lower() not in ("0", "false", "no", "off")
        time_injection = os.environ.get("TIME_INJECTION", "date" if cache_friendly else "minute").strip().lower()
        inject_silence = os.environ.get("INJECT_SILENCE_HOURS", "false" if cache_friendly else "true").strip().lower() in ("1", "true", "yes", "on")
        ...
        try:
            vector_memory_chars = int(os.environ.get("VECTOR_MEMORY_CHARS", "1200"))
        except Exception:
        ...
        elif time_injection in ("hour", "hours"):
            time_line = f"当前时间：{now_bj.strftime('%Y-%m-%d %H点')}左右（北京时间）。"
        else:
            # 默认 date：每天只变化一次，比每分钟变化更利于 cache 命中。
            time_line = f"当前日期：{now_bj.strftime('%Y-%m-%d')}（北京时间）。"

        # 沉默时长：默认缓存友好模式关闭；需要“多久没聊”再开。
        silence_hours = None
        if inject_silence:
            try:
        ...
        # 用户画像：相对稳定，放在前面，提高 prompt cache 可复用部分。
        user_prof = "暂无"
        try:
        ...
        # 最近对话历史：变化频繁，数量默认降到 6，减少 token 与缓存扰动。
        history_msgs = []
        if history_limit > 0:
            try:
        ...
        # 稳定在前，动态在后。这样 DeepSeek prompt cache 至少能稳定命中前半段。
        status_inject = "\n".join(stable_parts) + dynamic_part

        # 注入到 messages：已有 system 就追加，没有就插入。
        has_system = False
        for m in req_data.get("messages", []):
            if m.get("role") == "system":
                # 注意：把稳定内容放在追加内容的前面，但保留前端原 system 作为最前缀。
        ...
        # 把最近上文插到 system 之后、当前 user 之前。它是动态内容，所以放在稳定 system 之后。
        if history_msgs:
            sys_idx = 0
        ...
        _log(
            f"🧠 [智能体/缓存友好] 注入完成：画像{len(user_prof)}字 + "
