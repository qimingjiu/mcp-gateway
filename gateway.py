...
class HostFixMiddleware:
    ...
    async def _handle_chat(self, scope, receive, send):
        ...
        tools_value = req_data.get("tools", [])
        tools_count = len(tools_value) if isinstance(tools_value, list) else 0
        try:
            tools_text = json.dumps(tools_value, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
        except Exception:
            tools_text = f"<{type(tools_value).__name__}>"
        tools_hash = hashlib.sha256(tools_text.encode("utf-8")).hexdigest()[:12]

        def _diagnostic_param(value):
            try:
                return json.dumps(value, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
            except Exception:
                return f"<{type(value).__name__}>"

        _log(f"[cache-diagnostic] tools_count={tools_count}; tools_sha256={tools_hash}")
        _log(
            f"[cache-diagnostic] tool_choice={_diagnostic_param(req_data.get('tool_choice'))}; "
            f"thinking={_diagnostic_param(req_data.get('thinking'))}; "
            f"model={_diagnostic_param(req_data.get('model'))}"
        )
    ...
    async def _inject_context(self, req_data, sb, current_query):
        ...
        前端完整回传历史时，cache 可以随对话轮数增长，长对话命中率才可能持续上升。
        """
        ...
        def _short_sha256(value):
            return hashlib.sha256(_diagnostic_text(value).encode("utf-8")).hexdigest()[:12]

        incoming_messages = req_data.get("messages", [])
        if not isinstance(incoming_messages, list):
            incoming_messages = []
        roles = [str(m.get("role", "unknown")) if isinstance(m, dict) else "unknown" for m in incoming_messages]
        user_count = sum(role == "user" for role in roles)
        assistant_count = sum(role == "assistant" for role in roles)
        original_system = next(
            (m.get("content", "") for m in incoming_messages if isinstance(m, dict) and m.get("role") == "system"),
            "",
        )
        last_eight_lengths = [
            f"{(m.get('role', 'unknown') if isinstance(m, dict) else 'unknown')}:{len(_diagnostic_text(m.get('content', '') if isinstance(m, dict) else m))}"
            for m in incoming_messages[-8:]
        ]
        _log(
            f"[cache-diagnostic] messages={len(incoming_messages)}; roles={','.join(roles)}; "
            f"users={user_count}; assistants={assistant_count}; "
            f"last8_chars=[{','.join(last_eight_lengths)}]; "
            f"original_system_sha256={_short_sha256(original_system)}"
        )
        last_twelve_fingerprints = []
        for m in incoming_messages[-12:]:
            role = str(m.get("role", "unknown")) if isinstance(m, dict) else "unknown"
            content = m.get("content", "") if isinstance(m, dict) else m
            content_text = _diagnostic_text(content)
            content_hash = hashlib.sha256(content_text.encode("utf-8")).hexdigest()[:12]
            last_twelve_fingerprints.append(f"{role}:{len(content_text)}:{content_hash}")
        _log(f"[cache-diagnostic] last12=[{','.join(last_twelve_fingerprints)}]")

        last_user_before = next(
            (m.get("content", "") for m in reversed(incoming_messages) if isinstance(m, dict) and m.get("role") == "user"),
            "",
        )
        _log(f"[cache-diagnostic] last_user_before_sha256={_short_sha256(last_user_before)}")
        ...
        _log(
            f"[cache-diagnostic] user_facts_sha256={_short_sha256(user_prof)}; "
            f"core_cognition_sha256={_short_sha256(core_summaries)}; "
            f"stable_inject_sha256={_short_sha256(stable_inject)}"
        )
        ...
        # 下一轮前端通常只回传原始 user 内容，因此动态后缀不会占据下一轮历史前缀位置。
        # 这样 [system_stable, user_1, assistant_1, user_2, assistant_2, ...] 的稳定前缀
        # 不会被动态内容提前打断；前端完整回传历史时，缓存才会随对话轮数增长。
        #
        # 缓存增长示意：
        ...
        # 找最后一条 user 消息，把动态上下文直接后缀拼入其 content
        ...
        last_user_after = next(
            (m.get("content", "") for m in reversed(messages) if isinstance(m, dict) and m.get("role") == "user"),
            "",
        )
        _log(f"[cache-diagnostic] last_user_after_sha256={_short_sha256(last_user_after)}")
