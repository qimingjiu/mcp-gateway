...
class HostFixMiddleware:
    ...
    async def _handle_chat(self, scope, receive, send):
        ...
        # ==============================
        # DeepSeek 官方 API：缓存/思考模式实验开关
        # ==============================
        # 说明：
        # - user_id 用于 DeepSeek 侧 KVCache 隔离，不要填邮箱/手机号/真实姓名。
        # - DEEPSEEK_THINKING=disabled 可临时关闭 thinking，用来测试 reasoning_content 是否影响缓存命中。
        # - 测完如果想恢复思考，把 DEEPSEEK_THINKING 改成 enabled 或直接删掉这个环境变量。
        is_deepseek_upstream = (
            "deepseek" in (upstream_base or "").lower()
            or os.environ.get("DEEPSEEK_COMPAT", "").strip().lower() in ("1", "true", "yes", "on")
        )
        if is_deepseek_upstream:
            ds_user_id = (
                os.environ.get("DEEPSEEK_USER_ID")
                or os.environ.get("PINECONE_USER_ID")
                or os.environ.get("USER_ID")
                or ""
            ).strip()
            if ds_user_id:
                # DeepSeek 要求 user_id 不含隐私信息，字符集建议 [a-zA-Z0-9_-]
                ds_user_id = re.sub(r"[^a-zA-Z0-9_-]", "_", ds_user_id)[:512]
                req_data["user_id"] = ds_user_id

            ds_thinking = os.environ.get("DEEPSEEK_THINKING", "").strip().lower()
            if ds_thinking in ("disabled", "disable", "off", "false", "0", "no"):
                req_data["thinking"] = {"type": "disabled"}
                req_data.pop("reasoning_effort", None)
                _log("🧪 [DeepSeek] thinking=disabled，用于缓存命中对照实验")
            elif ds_thinking in ("enabled", "enable", "on", "true", "1", "yes"):
                req_data["thinking"] = {"type": "enabled"}
                ds_effort = os.environ.get("DEEPSEEK_REASONING_EFFORT", "").strip().lower()
                if ds_effort in ("high", "max"):
                    req_data["reasoning_effort"] = ds_effort
                _log(f"🧪 [DeepSeek] thinking=enabled, user_id={req_data.get('user_id', '(none)')}")
        ...
        # 缓存诊断：只打印 hash，不打印正文或密钥
        _log_deepseek_cache_probe(req_data, "before_upstream")

        if os.environ.get("DEBUG_CACHE_HASH", "").strip().lower() in ("1", "true", "yes", "on"):
            _log("🧪 [CacheProbe] DEBUG_CACHE_HASH=true，准备打印本轮请求前缀 hash")
