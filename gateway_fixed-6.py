...
SSE_BATCH_MAX_WAIT_SECONDS = 0.030
SSE_BATCH_MAX_BYTES = 4096
...
class HostFixMiddleware:
    ...
    async def _handle_chat(self, scope, receive, send):
        ...
        sse_line_count = 0
        received_done = False
        first_sse_forwarded = False
        sse_batch = bytearray()
        sse_batch_started_at = None
        largest_batch_bytes = 0
        heartbeat_deadline = time.perf_counter() + KEEPALIVE_INTERVAL_SECONDS

        async def _send_sse_body(body):
            nonlocal largest_batch_bytes
            largest_batch_bytes = max(largest_batch_bytes, len(body))
            return await _send_asgi({"type": "http.response.body", "body": body, "more_body": True})

        async def _flush_sse_batch():
            nonlocal sse_batch_started_at
            if not sse_batch:
                return True
            body = bytes(sse_batch)
            sse_batch.clear()
            sse_batch_started_at = None
            return await _send_sse_body(body)

        async def _forward_sse_frame(frame):
            nonlocal first_sse_forwarded, sse_batch_started_at
            if not first_sse_forwarded:
                first_sse_forwarded = True
                return await _send_sse_body(frame)

            if not sse_batch:
                sse_batch_started_at = time.perf_counter()
            sse_batch.extend(frame)
            if len(sse_batch) >= SSE_BATCH_MAX_BYTES:
                return await _flush_sse_batch()
            return True
        ...
        while True:
            ...
            if (
                sse_batch
                and sse_batch_started_at is not None
                and now - sse_batch_started_at >= SSE_BATCH_MAX_WAIT_SECONDS
            ):
                if not await _flush_sse_batch():
                    break
                continue

            try:
                chunk = async_queue.get_nowait()
                ...
                next_deadline = heartbeat_deadline
                if sse_batch and sse_batch_started_at is not None:
                    next_deadline = min(
                        next_deadline,
                        sse_batch_started_at + SSE_BATCH_MAX_WAIT_SECONDS,
                    )
                try:
            ...
            if isinstance(chunk, dict) and "error" in chunk:
                ...
                if not await _flush_sse_batch():
                    break
                if not await _send_sse_body(f"data: {json.dumps(err_chunk, ensure_ascii=False)}\n\n".encode("utf-8")):
                    break
                continue
            ...
            if chunk == "data: [DONE]":
                if not received_done:
                    ...
                    if not await _flush_sse_batch():
                        break
                    if not await _send_sse_body(b"data: [DONE]\n\n"):
                        break
                break
            ...
            sse_line_count += 1
            if not await _forward_sse_frame((chunk + "\n\n").encode("utf-8")):
                break
        ...
        average_lines_per_send = sse_line_count / asgi_send_count if asgi_send_count else 0.0
        _log(
            "[stream-diagnostic] "
            ...
            f"average_lines_per_send={average_lines_per_send:.2f}; "
            f"largest_batch_bytes={largest_batch_bytes}; "
